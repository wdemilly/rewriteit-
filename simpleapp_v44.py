...
v44 change vs v43 (UI rework — addresses the "fresh dashboard after run"
issue and restores v30s-style layout):

  PROBLEM v43 had: Streamlit reruns the script from the top on every
  user interaction, browser refresh, or websocket reconnect. v43's
  `result` was a local in main() and rebuilt each rerun. When the user
  returned to the page after a long run, run_button was False (the
  click event is one-shot), so st.stop() fired and the UI looked
  fresh — even when the run had completed and was on disk.

  v44 FIXES IT in two ways:

    (1) Last completed run cached in st.session_state["last_result"]
        and re-rendered on every script execution. So a browser refresh,
        tab switch, or websocket reconnect after a successful run shows
        the winning chapter and download buttons again — without
        re-running the pipeline.

    (2) Two-column body layout matching v36 (st.columns([1, 1])).
        Left column: Step 1 chapter outline upload + Run button +
        live log. Right column: Recent runs panel (always expanded),
        last completed run summary, winning chapter download, full-run
        zip download. This matches the v30s convention where prior
        results stay visible on the right while you set up the next
        run on the left.
...
def main():
    ...
    with left_col:
        ...
        run_button = st.button(
            "Run pipeline",
            type="primary",
            disabled=not (outline_text and api_key),
        )
