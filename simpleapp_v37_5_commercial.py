...
def scan_draft(text: str) -> dict:
    ...
    # v37.4a — scanner-veto carve-out for the three v36.32 patterns.
    # The Cap 19 anaphora, Cap 14 negation-cluster, and Cap 5 v2 meta-naming
    # detectors were calibrated against export_46's failure modes. Band-level
    # analysis of export_54 (Grimaldi Ch 2) showed those patterns landing on
    # Originality bands that are not the actual ORANGE driver — the longer
    # anaphoric "The Italian was..." cluster scored LIGHT_GREEN, while the
    # real ORANGE concentrations sat on enumeration/inventory passages that
    # no current scanner detects. Vetoing on the wrong patterns blocks ship
    # while leaving the actual driver untouched. v37.4a removes those three
    # lines from the conjunction so they stop blocking. The detectors below
    # (anaphora_hits / negation_cluster_hits / meta_naming_hits) remain
    # active; their counts continue to flow into scan_flagged_passages, the
    # runs.csv columns, and the UI summary for diagnostic value. Restoring
    # the veto is a single-block edit if a future export shows these
    # patterns are in fact what Originality flags.
    hard_cap_pass = (
...
def extract_text_from_upload(uploaded_file) -> str:
    ...
    return ""
# ============================================================================
# Packet-structure gate (v37.5)
# ============================================================================
# The create-outline step (create_outline_from_fiction_text) is supposed to
# emit a v21 drafting packet — a structured brief carrying POV/Setting/Time,
# numbered MICROBEATS, the CANONICAL CONTROL BLOCK (caps 1-20), and the
# CONTINUITY CHECKLIST. Every behavioral control the pipeline relies on lives
# INSIDE that structure.
#
# Failure mode this gate closes: the create-outline prompt is long and
# saturated with drafting-frame language ("the regenerated chapter," "the
# drafter," "write the chapter again"), so the extraction model can lock onto
# the source's register and emit FINISHED PROSE — a rewritten chapter — instead
# of a packet. simpleapp had no check for this: it would accept the prose as
# the "Outline" and draft from it with the bare system prompt and NONE of the
# caps, producing un-processed AI prose that scores warm and silently voids any
# test of the pipeline's constraints. A real packet contains dozens of these
# markers; a prose chapter contains zero. The gate is structural, not
# content-based, so it is book- and author-agnostic.
PACKET_MARKERS = [
    "BEGIN OUTLINE",
    "POV:",
    "MICROBEATS",
    "CANONICAL CONTROL BLOCK",
    "CONTINUITY CHECKLIST",
    "Dramatic action:",
    "Required details:",
]
# A genuine v21 packet hits all of these; finished prose hits none. The
# threshold is deliberately low so minor format drift in a real packet still
# passes, while the prose-misfire (zero markers) is always caught.
PACKET_MARKER_MIN = 3
def validate_packet_structure(outline_text: str) -> tuple:
    """Return (is_packet, present, missing).

    is_packet is True when at least PACKET_MARKER_MIN canonical markers are
    found. present/missing are the marker lists, for an actionable message.
    """
    text = outline_text or ""
    present = [m for m in PACKET_MARKERS if m in text]
    missing = [m for m in PACKET_MARKERS if m not in text]
    return (len(present) >= PACKET_MARKER_MIN, present, missing)
# ============================================================================
# API key loading
# ============================================================================
    ...
    while round_no < max_tries and not fatal_generation_error:
        ...
        for slot in open_slots:
            ...
            meta = {
                ...
                "source_outline_filename": outline_filename,
                "scan": scan_result,
            }
...
with st.sidebar:
    ...
    if outline_file:
        ...
        st.session_state["outline_source_filename"] = outline_file.name
    st.markdown("---")
    ...
    if st.button("Generate & Evaluate", type="primary", disabled=not api_key or total_runs == 0):
        ...
        if not doc_uploads:
            ...
            if "Outline" not in doc_uploads:
                ...
                _is_packet, _present, _missing = validate_packet_structure(
                    doc_uploads["Outline"]
                )
                if not _is_packet:
                    problems.append(
                        "Outline is not a v21 packet — it looks like finished prose. "
                        "The create-outline step likely emitted a chapter instead of "
                        "an outline, so NONE of the caps/controls are present and the "
                        "draft would be un-processed AI prose. Re-run "
                        "create_outline_from_fiction_text and confirm the output starts "
                        "with BEGIN OUTLINE and contains MICROBEATS, the CANONICAL "
                        "CONTROL BLOCK, and the CONTINUITY CHECKLIST. "
                        f"(Markers found: {', '.join(_present) if _present else 'none'}.)"
                    )
        ...
        if problems:
            ...
            for iter_idx in range(loop_iterations):
                ...
                gated = generate_quality_gated_batch(
                    ...
                    progress=progress,
                    status=status,
                    outline_filename=st.session_state.get("outline_source_filename", ""),
