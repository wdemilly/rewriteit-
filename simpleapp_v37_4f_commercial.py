...
# v37.4d cost lever: eval model moved from Opus 4.6 to Sonnet 4.6 ($3/$15 vs
# $5/$25 — ~1.7x cheaper on a large-input call that ingests every draft).
# eval_model drives selection (run_final_pass) and graft judgment (Stage D
# candidate-ID + commit) — NOT prose generation; the drafter stays Opus 4.7.
# The graft-improvement judgment is the craft-sensitive part and is backstopped
# by the deterministic _graft_preserves_specificity gate (v36.29/v36.30).
# Reversible: revert this constant or override in the UI field. Watch graft
# quality (not selection) against the next score run.
...
MAX_REGEN_ITERATIONS = 1      # v37.4f: was 3 (v37.4d: was 8). The regen loop
                              # re-runs full Opus batches (up to N_iters x repetitions
                              # drafts/chapter). Across all observed v37.4c/4e runs the
                              # loop has NOT produced a ship-band result — every batch
                              # ended Shipped:False / Needs regen:True — so every
                              # iteration past the first is Opus spend with no
                              # deliverable. Held at 1 (single batch, no auto-retry)
                              # until the ship gate actually ships. Tune up only after
                              # a regen iteration is shown to reach ship band.
                              # --- prior rationale (v37.4d, cap was 3) ---
                              # v37.4d set 3: scanner-veto restoration makes more
                              # drafts FAIL hard_cap_pass; at 8 the worst case was
                              # 8 x repetitions full Opus-4.7 drafts per chapter.
TEMP_BUMP_PER_ITER = 0.1      # temperature bump added on each regen iteration
TEMP_BUMP_CAP = 1.0           # Anthropic Messages API requires temperature in 0..1
# v37.4: Claude Opus 4.6/4.7 can reject the temperature parameter entirely
# ("temperature is deprecated for this model"). Leave this False for current
# Anthropic models; set True only if deliberately using an older model that
# still accepts temperature. Slot temperatures are retained as run labels only.
ANTHROPIC_SEND_TEMPERATURE = False
...
def scan_draft(text: str) -> dict:
    ...
    # v37.4c — RESTORATION of the three v36.32 scanner-veto lines that v37.4a
    # removed. Empirical basis: the May 27 four-source CEILING CHECK (TLR /
    # Grimaldi / Boris / Quiet Part on v37.4b + create_outline_v6) shipped
    # every draft with scan_hard_cap_pass = False because the v37.4a removal
    # made hard_cap_pass blind to anaphora / negation_cluster / meta_naming.
    # Boris in particular scored 54 with 9 the_way hits AND 12 anaphora hits
    # in 2,452 words (4.9/k anaphora — radically outside any commercial-
    # fiction baseline); the scanner caught it, the gate let it through.
    # Meanwhile the May 17 v36.25 + normalize_v10H run on TLR Ch 4 scored 94
    # against this same v37.4b stack's 82 on the same chapter — a 12-point
    # regression in 10 days whose only candidate cause is the v37.4a veto
    # removal (v36.25 had those three lines enforcing the cap; v37.4a
    # stripped them; v37.4b inherited the strip). v37.4c puts them back. The
    # original v37.4a comment cited export_54 band-data on Grimaldi as
    # justification for the strip; subsequent evidence shows the patterns
    # DO drive warm-share on register-distant content (Boris, Quiet Part)
    # even when they don't on Tide-Walkers-register content (TLR/Grimaldi).
    # Generalizes across all inputs; no per-register branching.
    hard_cap_pass = (
...
def clamp_anthropic_temperature(value, default: float = 0.9) -> float:
    """Return a temperature value accepted by Anthropic's Messages API.

    Anthropic rejects temperatures outside 0..1. The regeneration loop can
    still increase variety, but it must never pass 1.0 to the API. This helper
    also protects against sidebar input such as 1.2 or blank/non-numeric values.
    """
    try:
        t = float(value)
    ...
    if not np.isfinite(t):
        t = float(default)
    return max(0.0, min(round(t, 3), TEMP_BUMP_CAP))


def normalize_temperature_list(values) -> list:
    """Clamp, round, and de-duplicate temperatures while preserving order."""
    cleaned = []
    seen = set()
    for value in values or []:
        t = clamp_anthropic_temperature(value)
        key = f"{t:.3f}"
        if key not in seen:
            seen.add(key)
            cleaned.append(t)
    return cleaned or [0.9]
...
def build_message_blocks(prompt_text: str, doc_texts: dict) -> list:
    ...
    blocks.append({
        ...
        # v37.4f: prompt-cache breakpoint. The entire generation prefix
        # (system prompt + P63 prompt + packet/doc blocks + this final
        # instruction) is byte-identical across every repetition of a chapter
        # and across any regen iteration; sampling is the only thing that
        # varies. A single breakpoint on this last user block caches that whole
        # prefix, so every draft after the first within the cache TTL reads the
        # packet at ~10% of input price. Billing-only: does not change any
        # generated token. Packet size keeps the prefix well above the Opus
...
def generate_chapter(client, model: str, temperature: float, message_blocks: list) -> str:
    ...
    v37.4: Do not send `temperature` by default. Current Claude Opus models
    can reject the parameter entirely with "temperature is deprecated for this
    model." The temperature value is still preserved in filenames, run records,
    and UI labels so old batch accounting stays stable, but it is not passed to
    Anthropic unless ANTHROPIC_SEND_TEMPERATURE is deliberately set True.
    """
    request_kwargs = {
        "model": model,
        "max_tokens": MAX_GEN_TOKENS,
    ...
    if ANTHROPIC_SEND_TEMPERATURE:
        request_kwargs["temperature"] = clamp_anthropic_temperature(temperature)
    try:
        resp = client.messages.create(**request_kwargs)
    except Exception as exc:
        # Safety net for older local copies where ANTHROPIC_SEND_TEMPERATURE
        # may be toggled on or where a model changes policy. Retry once
        # without temperature only for the explicit deprecation error.
        msg = str(exc).lower()
        if "temperature is deprecated" in msg or "temperature" in msg and "deprecated" in msg:
...
def generate_quality_gated_batch(
    ...
    """
    temperatures = normalize_temperature_list(temperatures)
    ...
    if not LOCAL_SCORER_AVAILABLE:
        ...
        try:
            ...
            if not pre_text.strip():
                ...
                result["local_score"]["pre_hard_cap_summary"] = format_scan_summary(pre_scan)
                ...
                if pre_score >= SCORE_SHIP and pre_hard_cap_pass:
                    ...
                    result["local_score"]["post_hard_cap_summary"] = format_scan_summary(post_scan)
                    ...
                    if post_score >= SCORE_SHIP and post_hard_cap_pass:
                        ...
                        # v37.4e (Candidate C-2): Stage G changed the text and did
                        # not clear SHIP. The local-scorer pre/post delta is NOT
                        # trusted to decide whether to keep the edits. On the
                        # May-27 four-source test the local scorer said REVERT on
                        # Boris (post 65.9 < pre 68.7) while real Originality turbo
                        # had the LINEEDITED text at +7 over GRAFTED (61 vs 54),
                        # and LINEEDITED was net +6 across the three runs where
                        # Stage G fired (Boris +7, Quiet Part +1, TLR -2). The old
                        # STAGE_G_REVERT_REGEN branch threw that gain away on the
                        # scorer's unreliable say-so. So whenever Stage G produced
                        # changed text, keep it as the ship file. post_g_score
                        # still drives needs_regen for the regen loop; it no longer
                        # drives the ship-FILE choice.
                        result["final_text_lineedited"] = edited_text_candidate
                        result["local_score"]["decision"] = "STAGE_G_KEEP_EDITS_REGEN"
                        result["local_score"]["shipped_text_source"] = "final_text_lineedited"
                        result["needs_regen"] = True
                    else:
                        # Stage G ran but changed nothing — no edited text to keep.
                        result["local_score"]["decision"] = "STAGE_G_NOOP_REGEN"
                        result["local_score"]["shipped_text_source"] = "final_text"
                        result["needs_regen"] = True
                else:
            ...
            result["needs_regen"] = True
            result["shipped"] = False
    # v37.4e (Candidate C-2): make the canonical deliverable follow the ship
    # decision. final_path was set to the GRAFTED (or UNGRAFTED) file earlier,
    # before Stage G ran. When the local-scorer flow elected to ship the
    # line-edited text (STAGE_G_THEN_SHIP or STAGE_G_KEEP_EDITS_REGEN), repoint
    # final_path to the LINEEDITED file so the SUMMARY "Final deliverable" line,
    # the GitHub push list, and the download UI all designate the file the
    # pipeline actually chose — not the GRAFTED file final_path defaulted to.
    # The GRAFTED file remains in the bundle, reported separately as
    # "Grafted winner", so GRAFTED vs LINEEDITED stays directly comparable.
    if (result.get("local_score", {}).get("shipped_text_source")
            == "final_text_lineedited"):
        _le_path = (result.get("line_edit") or {}).get("edited_path")
        if _le_path:
            result["final_path"] = str(_le_path)
            result["final_source"] = "top1_lineedited"
    try:
        predictor = stage_f_load_predictor(str(LABELED_CORPUS_PATH), STAGE_F_RIDGE_LAMBDA)
        batch_scores = []
        for d in drafts:
...
with st.sidebar:
    ...
    st.markdown("---")
    temps_input = st.text_input("Variation slots / legacy temperatures (comma-separated)", value="0.9")
    try:
        raw_temperatures = [float(t.strip()) for t in temps_input.split(",") if t.strip()]
    except ValueError:
        raw_temperatures = [0.9]
        st.warning("Could not parse variation slots. Using 0.9.")
    temperatures = normalize_temperature_list(raw_temperatures)
    if raw_temperatures and temperatures != raw_temperatures:
        st.warning(
            "Variation-slot values were clamped to the legacy 0..1 range: "
            f"{temperatures}"
        )
    ...
    st.caption(f"Variation slots: {temperatures} · Reps: {repetitions} · Top-N: {top_n}")
    if doc_uploads:
                ...
                if len(batch_drafts) >= 1:
                    ...
                    try:
                        ...
                        if decision == "SHIP":
                            ...
                            iter_score = float(pre_s) if pre_s is not None else -1.0
                        elif decision == "STAGE_G_NOOP_REGEN":
                            # v37.4e: Stage G ran but changed nothing; the iter
                            # offers the pre-G grafted text, scored at pre_s
                            # (same as the retired REVERT case).
                            iter_score = float(pre_s) if pre_s is not None else -1.0
                        elif decision == "REGEN":
                            iter_score = float(pre_s) if pre_s is not None else -1.0
                        else:
                        ...
                        if iter_idx < loop_iterations - 1:
                            current_temperatures = normalize_temperature_list(
                                [t + TEMP_BUMP_PER_ITER for t in current_temperatures]
                            )
                    except Exception as e:
                        import traceback
