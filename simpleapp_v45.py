...
v45 change vs v44 (Step 5 prompt selection restored from prompts.csv):

  PROBLEM v44 had: the Step 5 drafting prompt was hardcoded as
  draft_chapter_prompt_v1.txt — a ~600-word file with self-suppression
  language ("SUPPRESS YOUR OWN TEMPLATES... your own ideas about how to
  construct prose are not the standard"), five "design rules" that
  smuggled prohibition caps inside positive-sounding wording, and an
  explicit per-sentence compliance loop. That produced wooden prose at
  Step 5, which then failed downstream gates.

  v45 RESTORES the prompts.csv selection mechanism that pre-v40 simpleapp
  used. The operator picks a drafting prompt by ID from prompts.csv at
  run time. The selected prompt text is prepended to the OUTLINE and
  AUTHOR_CONSTRUCTION blocks (same template structure v44 used) and
  becomes the Step 5 instruction.

  draft_chapter_prompt_v1.txt is no longer referenced. The file can
  remain on disk unused; v45 does not load it.

  prompts.csv lives in APP_DIR next to simpleapp_v45.py. Schema:
  id,category,text. The operator can add rows freely.
...
ARCHITECTURE (per architecture_record_v1.txt):
  ...
  Step 5.  Generate X drafts (default X=3) of the chapter from the
           outline + AUTHOR_CONSTRUCTION + operator-selected prompt.
           Same temperature; variation from sampling.
...
DEPENDENCIES (must be co-located in the same directory):
  ...
  prompts.csv                 (v45: operator-selectable Step 5 prompts)
  rule_verifier_v1.py
...
USAGE:
  streamlit run simpleapp_v45.py
"""
...
#               Should NOT contain {outline} or {author_construction}
#               placeholders — the assembler appends those sections below.
#
...
def build_draft_prompt(
    ...
    Structure (matches the section markers v44's draft_chapter_prompt_v1.txt
    placed below its template body, so existing AUTHOR_CONSTRUCTION
    ...
    instruction. v45 deliberately adds no other framing, design rules,
    compliance language, or self-suppression directives.
    """
    return (
        f"{csv_prompt_text.strip()}\n\n"
        f"=== OUTLINE ===\n\n{outline_text.strip()}\n\n"
        f"=== AUTHOR_CONSTRUCTION ===\n\n{author_construction.strip()}\n"
    )
...
def generate_draft(
    ...
    v45: the drafting instruction is csv_prompt_text (selected by the
    operator from prompts.csv). build_draft_prompt assembles the full
    user-message body."""
    prompt = build_draft_prompt(csv_prompt_text, outline_text, author_construction)
...
def generate_draft_batch(
    client, model, outline_text, author_construction, csv_prompt_text,
    n_drafts, log,
    ...
    for i in range(n_drafts):
        ...
        try:
            d = generate_draft(
                client, model, outline_text, author_construction,
...
def _render_last_result_panel() -> None:
    ...
    with st.expander("Author construction block (step 2-3)"):
        st.code(result.get("author_construction", ""), language="text")
...
def _load_last_terminal_result_from_disk() -> Optional[dict]:
    ...
    for m in recent:
        ...
        # Rehydrate author construction
        ac_path = run_dir / "author_construction.txt"
        author_construction = ac_path.read_text(encoding="utf-8") if ac_path.exists() else ""
...
def main():
    ...
    st.session_state["last_result"] = {
        ...
        "author_construction": result["author_construction"],
